...
scene *scene_get_current() { return current_scene; }

void scene_create(scene *S) { S->entity_count = 0; }

bool scene_add_entity(scene *S, entity *entity) {
	printf("checking to see if can add entity to sceen\n");
  if (S->entity_count < SCENE_MAX_ENTITY_COUNT) {
	printf("can add entity to sceen\n");
    S->entitys[S->entity_count] = entity;
    S->entity_count++;
    return true;
  }
	printf("can add entity to sceen\n");
...
void scene_run_lua(scene *s, lua_State *L) {
  ...
  if (len != prev_len) {
    ...
    if (s->entity_count > 0) {
      ...
      for (int i = 0; i < s->entity_count; i++) {
        Render::add_to_buffer(s->R, &s->entitys[i]->sprite);
      }
    }
	printf("count is 0\n");
  }
